...
def main():
    ...
    for n, record in enumerate(screed.open(transcript_file)):
        ...
        o = ortho.get(name)
        if o and transform_name(o) in namedb.uniprot_names:
            annot = namedb.uniprot_names[transform_name(o)]
            ...
            if h:
                ...
                annot = namedb.uniprot_names.get(transform_name(h))
    ...
    for n, record in enumerate(screed.open(transcript_file)):
        ...
        if o and transform_name(o) in namedb.uniprot_names:
            ...
            if h and transform_name(h[0][0]) in namedb.uniprot_names:
                h, score = h[0]
                score = round(float(score) / float(len(record.sequence)) * 100)
                annot = namedb.uniprot_names[transform_name(h)]
